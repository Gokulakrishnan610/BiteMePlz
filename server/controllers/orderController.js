...
const getOrderByPaymentId = asyncHandler(async (req, res) => {
  // For Supabase, we need to manually filter since it doesn't support nested field queries
  const allOrders = await OrderService.find({ is_paid: true });
  const orders = allOrders.filter(order => 
    order.payment_result && 
    order.payment_result.razorpay_payment_id === req.params.paymentId
  );
...
const verifyOrderQR = asyncHandler(async (req, res) => {
  ...
  const order = await OrderService.findById(qrPayload.order_id);
